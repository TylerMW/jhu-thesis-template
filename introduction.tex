...
rotating wires depends on both the interfacial and subphase properties, and the relative importance of these contributions can be parametrized by the Boussinesq number. According to recent theories by Levine et al.48,49 and by Dhar et al.,50 dealing with the hydrodynamic drag on a highly anisotropic object, such as a wire-shaped colloid, the characteristic length scale entering the Boussinesq number is set by the wire length L UP UNTIL IT BECOMES SMALL, RIGHT?. Specifically, Levine and co-workers predict that the rotational drag coefficient $\zeta_r$ approaches $1.48\eta_s L^2$ for $L/l_0 \ll 1$ and $0.50\eta L^3$ for $L/l_0 \gg 1$, respectively. They also calculate a form for $\zeta_r$ as a function of $L/l_0$ that interpolates between these limits and provide predictions for the translational drag coefficients. Measurements on Ni nanowires within viscous protein layers comparing the drag for translation parallel to the wire axis $\zeta_\parallel$
 with $\zeta_r$ indicate that the results of Levine et al. accurately predict $\zeta_\parallel/\zeta_r$ and we use their theory to analyze the wire rotation.
