...
\noindent where $\epsilon$ is simply inverse Boussinesq number $1/\text{Bo}$ and the numerically-determined constants are $c_1 = 0.73761$, $b_1 = 2.74819$, $c_2 = 0.52119$ and $b_2= 0.61465$.
    ...
    \caption{\label{fig:drag-models}A comparison of models for the translation drag on a membrane inclusion. For this illustration, $\eta_1=10^{-3}$ Pa$\cdot$s and $\eta_2$ is negligible, as at an air--water interface. The radius of the inclusion $a=0.5$ \textmu m.}
    \end{figure}

A more elaborate model, the Fischer model, accounts for the contact angle of a spherical inclusion and the effort of Marangoni forces---surface tension gradidents produced by concentration gradients across the interface. At Bo $\ll 1$, an expansion in Bo gives the translational drag coefficient $\zeta$ as \cite{Fischer2006}
