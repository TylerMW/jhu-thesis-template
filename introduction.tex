...
\noindent A theory developed by Levine and MacKintosh predicts\cite{Levine2004} rotational drag coefficient for all Boussinesq numbers. Measurements on Ni nanowires within viscous protein layers comparing the drag for translation parallel to the wire axis $\zeta_\parallel$
 with $\zeta_r$ indicate that the results of this theory accurately predict $\zeta_\parallel/\zeta_r$,\cite{Lee2010} and we use their theory to analyze the wire rotation. Specifically, we employ their predictions in the limit of high and low Bo: $\zeta_r \rightarrow 1.48\eta_s L^2$ and $\zeta_r \rightarrow 0.50\eta L^3$, respectively.
